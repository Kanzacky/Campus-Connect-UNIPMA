...
interface Organisasi {
    id: number; name: string; category: string; ketua: string; visi: string; misi: string; deskripsi: string; kontak: string; status: string;
}
...
export default function OrganisasiEdit({ organisasi, categories }: { organisasi: Organisasi; categories: string[] }) {
    const { data, setData, put, processing, errors } = useForm({
        name: organisasi.name, category: organisasi.category, ketua: organisasi.ketua, visi: organisasi.visi, misi: organisasi.misi, deskripsi: organisasi.deskripsi || '', kontak: organisasi.kontak || '', status: organisasi.status,
    });

    function handleSubmit(e: React.FormEvent) {
        e.preventDefault();
        put(`/admin/organisasi/${organisasi.id}`);
    }

    return (
        <AppLayout breadcrumbs={breadcrumbs}>
            <Head title={`Edit ${organisasi.name}`} />
            <div className="mx-auto max-w-3xl p-4 md:p-6">
                <h1 className="mb-6 text-2xl font-bold text-zinc-900 dark:text-zinc-100">Edit Organisasi</h1>
                <form onSubmit={handleSubmit} className="space-y-6 rounded-xl border border-zinc-200 bg-white p-6 shadow-sm dark:border-zinc-800 dark:bg-zinc-900">
                    ...
                    <div className="flex items-center justify-end gap-3 border-t border-zinc-200 pt-6 dark:border-zinc-800">
                        <Link href="/admin/organisasi" className="rounded-lg px-4 py-2.5 text-sm font-medium text-zinc-700 hover:bg-zinc-100 dark:text-zinc-300">Batal</Link>
